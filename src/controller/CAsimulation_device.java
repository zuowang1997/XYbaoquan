...
public class CAsimulation_device extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public CAsimulation_device() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String mi_after1Hash = String.valueOf(request.getSession().getAttribute("mi_after1Hash_device"));
		...
		try {
			...
			System.out.println("CA收到来源指纹："+ming_Hashcode);
		} catch (Exception e) {
		...
		if(ca.checkHashCode(ming_Hashcode)){ 
			...
			boolean res_device = ca_time.addFileFingerprint_Device(userid,securitytime,after2Hash);
			boolean res_device2 = ca_time.addTimestamp_Device(after2Hash,unixtimestmap);
			if((res_device != false)&(res_device2 != false)){
		    	 System.out.println("CA成功添加时间保全记录到已有的数据库条目中!");
		    }
		}
		else{	} 
		...
		session.setAttribute("afterCA_Footprint_device", mi_afterCA_Footprint); 
