...
public class CA_yanzheng_All extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public CA_yanzheng_All() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String mi_Hashcode_time = String.valueOf(request.getSession().getAttribute("yanzheng_mi_All_after1Hash_time"));
		String mi_Hashcode_device = String.valueOf(request.getSession().getAttribute("yanzheng_mi_All_after1Hash_device"));
		String mi_Hashcode_data = String.valueOf(request.getSession().getAttribute("yanzheng_mi_All_after1Hash_data"));
		String mi_userid = String.valueOf(request.getSession().getAttribute("yanzheng_mi_All_userid"));
		String mi_securitytime = String.valueOf(request.getSession().getAttribute("yanzheng_mi_All_setime"));
		String after2Hash_time = null;
		String after2Hash_device = null;
		String after2Hash_data = null;
		...
		String ming_Hashcode_data = null;
		...
		try {
			...
			ming_Hashcode_data = rsa.decryptByPrivateKey(mi_Hashcode_data, priKey);
			...
			System.out.println("CA收到数据指纹："+ming_Hashcode_data+"用户ID："+ming_userid+"保全时间："+ming_securitytime);
		} catch (Exception e) {
		...
		*/
		...
		String Timestamp_data = sqlca.getTimeStampbyKey(ming_userid,ming_securitytime);
		...
		//检查哈希值是否是128位的
		if((ca.checkHashCode(ming_Hashcode_time))&&(ca.checkHashCode(ming_Hashcode_device))&&(ca.checkHashCode(ming_Hashcode_data))){ 
        ...
        String mi_afterCA_Footprint_data = null;
        try {
			...
			mi_afterCA_Footprint_data = rsa2.encryptByPublicKey(after2Hash_data, pubKey2);
			mi_unixTimestamp = rsa2.encryptByPublicKey(Timestamp_data, pubKey2);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		session.setAttribute("yanzheng_all_afterCA_Footprint_time", mi_afterCA_Footprint_time); 
		session.setAttribute("yanzheng_all_afterCA_Footprint_device", mi_afterCA_Footprint_device); 
		session.setAttribute("yanzheng_all_afterCA_Footprint_data", mi_afterCA_Footprint_data); 
		session.setAttribute("yanzheng_all_afterCA_timestamp", mi_unixTimestamp); 
