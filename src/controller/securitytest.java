...
public class securitytest extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		    ...
		    String filetype = FileNameWithFormat.substring(FileNameWithFormat.length()-4,FileNameWithFormat.length());
		    filename = FileNameWithFormat.substring(0,FileNameWithFormat.length()-4);
		    
		    
		    //用rsa和hash验证数据
		    /*
		    String get_data = userID + filename + securitysource + certificateholder + securitytype + FileNameWithFormat;
		    String get_data_hash = newcertificate.doHash(get_data);
		    String real_hash_encryped = String.valueOf(request.getSession().getAttribute("security_hash"));
		    RSA hashrsa = new RSA();
		    String real_hash = real_hash_encryped;
		    if(!(get_data_hash.equals(real_hash))){
		    	response.addHeader("Content-Type", "text/html");
            	response.setCharacterEncoding("UTF-8");
    		    response.getWriter().write("数据在中途遭到篡改！");
    		    System.exit(0);
		    }
		    else{  }*/
		    ...
		    if( WORDorPIC.equals(".txt") )
		    {
		    	...
		    	yanzheng DealWithWord = new yanzheng();
		    	File wordfile = new File("C:\\Tomcat\\webapps\\XYbaoquan\\upload\\"+FileNameWithFormat);
		    	String wordfile_string = DealWithWord.FiletoString(wordfile);
		    	securityString = wordfile_string;
		    ...
		    else if( WORDorPIC.equals(".pdf") )
		    {
		    	...
		    	String string_pdffile = null;
		    	make_certificate DealWithPDF = new make_certificate();
		    	File pdffile = new File("C:\\Tomcat\\webapps\\XYbaoquan\\upload\\"+FileNameWithFormat);
		    	int pagenumber = DealWithPDF.setup(pdffile);
		    	
		    	String midhash = null;
		    	for(int number=1; number<=pagenumber ; number++){
		    	File midfile = new File("C:/"+number+".jpg");
		    	string_pdffile = DealWithPDF.PictureToString(midfile);
		    	midhash = midhash + string_pdffile;
		    	midhash = newcertificate.doHash(midhash);
		    	}	
		    	securityString = midhash;
		    	for(int number=1; number<=pagenumber ; number++){
		    		newcertificate.deleteFile("C:/"+number+".jpg");
		    	}
		    	newcertificate.deleteFile("C:\\Tomcat\\webapps\\XYbaoquan\\upload\\"+FileNameWithFormat);
		    }
		    else if( (WORDorPIC.equals(".jpg")) | (WORDorPIC.equals(".png")) )
		    {
		    	...
		    	File picturefile = new File("C:\\Tomcat\\webapps\\XYbaoquan\\upload\\"+FileNameWithFormat);
		    	String picturefile_string = DealWithPicture.PictureToString(picturefile);
		    	securityString = picturefile_string;
		    	//System.out.println(picturefile_string);
		    	newcertificate.deleteFile("C:\\Tomcat\\webapps\\XYbaoquan\\upload\\"+FileNameWithFormat);
		    }
		    else{
		    	System.out.println("要保全的内容没有接收到啦！");
		    }
		    ...
		    session.setAttribute("securitytime", SecurityTime);
		  ...
		  //如果选择了保全来源
		    ...
		    //需要将空气中是否已经支付的flag位变回“否”23333
		    session.setAttribute("ZhiFu_Result", "fail");
		    //返回
